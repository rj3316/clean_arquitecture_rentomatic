...
def test_repository_read_all(room_dicts):
    # Instanciamos RepoSql y escribimos una lista obtenida desde la "variable mock" room_dicts
    repo = RepoFactory.create('RepoSql', {'db': mysql})
    repo.write(room_dicts)
    ...
    expected_rooms = [Room.from_dict(i) for i in room_dicts[domain]]
